...
function SC_PayFineEvent:new(fineHeight,vehicle)
...
function SC_PayFineEvent:run(connection)
    if not connection:getIsServer() then
        if self.vehicle~=nil then
            g_currentMission:addMoney(self.fine, self.vehicle:getActiveFarm(), MoneyType.OTHER, true);
            print("addMoney ausgeführt");
        else
            print("kein vehicle")
        end
    else
        print("Fehler bei run-Mathode");
    end
end
